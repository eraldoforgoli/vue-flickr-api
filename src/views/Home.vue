  ...
  <div>
    <nav class="navbar">
      <form class="searchbar">
        <label>
          ...
          <input v-model="tag" type="text" placeholder="Search for photos" class="searchbar-input">
        </label>
    ...
    <div class="wrapper">
      ...
      <ul v-else class="image-card-grid">
        <image-card v-for="image in cleanImages" :key="image.id" :image="image"></image-card>
      </ul>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    search() {
      ...
      console.log("key: " + process.env.VUE_APP_FLICKR_API_KEY);
      this.loading = true;
